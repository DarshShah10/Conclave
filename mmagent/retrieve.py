...
def generate_action(question, knowledge, retrieval_plan=None, multiple_queries=False, responses=[], switch=False, model="gpt-5-mini"):
    # select prompt
    if not switch:
        if multiple_queries:
...
def answer_with_retrieval(video_graph, question, video_clip_base64=None, topk=5, auto_refresh=False, mode='max', multiple_queries=False, max_retrieval_steps=10, route_switch=True, threshold=0, model="gpt-5-mini", before_clip=None):
    if before_clip is not None:
...
def verify_qa(question, gt, pred, model="gpt-5-mini"):
    try:
        input = [
            {
                "type": "text",
