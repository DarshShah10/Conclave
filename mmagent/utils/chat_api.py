...
try:
    ...
    for model_name in config.keys():
        # This checks if a base_url is provided (like for Gemini), 
        # otherwise it uses the default OpenAI path.
        if "base_url" in config[model_name] and config[model_name]["base_url"]:
...
def generate_messages(inputs):
    ...
    for input in inputs:
        ...
        if input["type"] == "text":
            ...
            # Some Gemini versions through OpenAI proxy prefer this format
            content.append(
                {
                    "type": "image_url",
